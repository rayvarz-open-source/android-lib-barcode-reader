...
public class BarcodeReaderFragment extends Fragment implements View.OnTouchListener, BarcodeGraphicTracker.BarcodeGraphicTrackerListener , CameraSourcePreview.CameraPreviewStatusUpdateListener {
    ...
    public static BarcodeReaderFragment newInstance(boolean autoFocus, boolean useFlash, boolean useCroppedFrame) {
        Bundle args = new Bundle();
        args.putBoolean(KEY_AUTO_FOCUS, autoFocus);
        args.putBoolean(KEY_USE_FLASH, useFlash);
        args.putBoolean(KEY_USE_CROPPED_FRAME, useCroppedFrame);
        args.putInt(KEY_SCAN_OVERLAY_VISIBILITY, useCroppedFrame? View.VISIBLE: View.INVISIBLE);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (arguments != null) {
            ...
            this.useCroppedFrame = arguments.getBoolean(KEY_USE_CROPPED_FRAME, false);
            this.scanOverlayVisibility = arguments.getInt(KEY_SCAN_OVERLAY_VISIBILITY, View.VISIBLE);
            if (useCroppedFrame) this.scanOverlayVisibility = View.VISIBLE;
        }
    }
    ...
    private void createCameraSource(final boolean autoFocus, final boolean useFlash) {
        ...
        if (useCroppedFrame) {
            barcodeDetector = new BoxDetector(barcodeDetector);
        }
        ...
        CameraSource.Builder builder = new CameraSource.Builder(getActivity(), barcodeDetector).setFacing(CameraSource.CAMERA_FACING_BACK).setRequestedPreviewSize(width, height).setRequestedFps(30.0f);
    ...
    private Rect createCroppingRect(Size parentSize) {
        ...
        int left = parentWidth / 4;
        int right = left * 3;
        if (mScanOverlay != null) {
            left = Math.round((parentHeight / 2) - (mScanOverlay.getRectHeight() / 3));
            right = Math.round((parentHeight / 2) + (mScanOverlay.getRectHeight() / 3));
        }
        int topMargin = Math.round(0);
        int bottomMargin = Math.round(parentSize.getHeight());
        int leftMargin = Math.round(left);
        int rightMargin = Math.round(right);
